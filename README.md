...
기존의 스프링 레거시와 Mybatis로 구성된 프로젝트를 Spring Boot와 JPA 등 최신 기술로 성공적으로 마이그레이션하여 93% 성능 개선과 함께 배포 프로세스 개선으로 CI/CD 무중단 배포를 이뤄냈습니다.

모든 API를 JSON으로 통신하면서 타 프론트엔드 프레임워크와의 확장성을 염두해두었고, 보안이 중요한 결제 프로세스와 로그인 시스템에 Spring Security와 AOP, DTO를 통해서 검증 프로세스를 강화했습니다.
